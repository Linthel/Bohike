...
{
    public class TikiTorcher : Enemy
    {
        public Torch Torch { get; set; }
        public ShadowWhirl ShadowWhirl { get; set; }
        public Spirit Spirit { get; set; }

        private float _torchDelay = 5f;
        private float _spiritDelay;
        private int _spiritCount = 1;
        ...
        public TikiTorcher(Texture2D texture)
          : base(texture)
        {
            ...
            if (_enemyState == EnemyStates.None)
            {
                if (AI.Attack && CanAttack)
                {
                    if (Health <= 20)
                    {
                        ...
                        InAttack = true;
                        _attackPower -= (float)gameTime.ElapsedGameTime.TotalSeconds * 0.25f;
                        _torchDelay -= (float)gameTime.ElapsedGameTime.TotalSeconds;
                        _spiritDelay -= (float)gameTime.ElapsedGameTime.TotalSeconds;

                        AddHurtbox(1f, HurtboxTypes.ShadowWhirl);
                        AddExplosion(ExplosionTypes.FireWhirl);

                        if (_spiritCount > 0 && _spiritDelay <= 0)
                        {
                        ...
                        if (_torchDelay <= 0)
                        {
                            ...
                            SoundManager.PlaySoundEffect(21);
                            _torchDelay = 0.6f;
                        }
                    }
                    else if (Health <= 40)
